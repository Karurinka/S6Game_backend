pipeline {
    ...
    stages {
		...
		stage('Kubectl magic'){
			steps {
				sh 'kubectl set image deployment/mycontainerregistry1610.azurecr.io/auth-service:kube${BUILD_NUMBER} --kubeconfig /users/gebruiker/.kube/config'
				sh 'kubectl set image deployment/mycontainerregistry1610.azurecr.io/lobby-service:kube${BUILD_NUMBER} --kubeconfig /users/gebruiker/.kube/config'
			}
		}
    }
}